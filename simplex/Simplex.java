...
public class Simplex{
  ...
  public static void main(String[] args){
    ...
    for(int i = 0; i < num_problems; i++){
      ...
      int[] btrans = new int[m];
      for(int j = 0; j < n; j++){
        ctrans[j] = scan.nextInt();
      }
      for(int j = 0; j < m; j++){
        btrans[j] = scan.nextInt();
      }
      int[][] constraints = new int[m][n];
      // System.out.println(m + " " + n);
      for(int j = 0; j < m; j++){
        for(int k = 0; k < n; k++){
          constraints[j][k] = scan.nextInt();
          // System.out.print(j + " " + k + " " + constraints[j][k] + " " );
        }
      }
      // if (i != 0)
      simplex(constraints, btrans, ctrans);
    }
  }
